...
OUTPUT_FOLDER = Path("/leonardo_work/IscrC_FoSAM-X/fetalus-fm/dinov3_pca_brain_analysis")
...
assert IMG_SIZE % PATCH_SIZE == 0, "The image size must be exactly divisible by <PATCH_SIZE>"
...
dinourl = 'https://dinov3.llamameta.net/dinov3_vitl16/dinov3_vitl16_pretrain_lvd1689m-8aa4cbdd.pth?Policy=eyJTdGF0ZW1lbnQiOlt7InVuaXF1ZV9oYXNoIjoidG81eWE0NDZjd28zMTJwamhjeGoxMWIyIiwiUmVzb3VyY2UiOiJodHRwczpcL1wvZGlub3YzLmxsYW1hbWV0YS5uZXRcLyoiLCJDb25kaXRpb24iOnsiRGF0ZUxlc3NUaGFuIjp7IkFXUzpFcG9jaFRpbWUiOjE3NTc1NzUzNjF9fX1dfQ__&Signature=Owb5pYpFJcAWTs3x6yt%7EMtUxrlFEcK5TkPzpnEYmVh5snZR8tefVBMzgJUQIAd305v%7EU3Xfy0MlYCeT-O47qqTmEMKiJ-vHzKmISLDmtMsh9lhsCP882QhiSHx%7EU35pD9VBfo-p1paHfOguyGdrlsEm0xy-oy4JfXN9vP4A1g9Mt6EsWXk4FlEbrJtHq8kMFCnrH6M-f7CLXhLdx%7EJvbhgdQLGFX160qBfP2aeRNoOb2gIXFSXCA%7EmFqMkMLnD67kx6hG3fURVS3DzYy9H3Pz3T7%7Ehgr8exvaZ7AzH7rlQWC3qLf4jDEzl0cYcRUyegICaKlWMLZbrFrEbRHk3QLTQ__&Key-Pair-Id=K15QRJLYKIFSLZ&Download-Request-ID=1554753395493379'
dinov3_vitl16 = torch.hub.load('/leonardo_work/IscrC_FoSAM-X/fetalus-fm/dinov3', 'dinov3_vitl16', source='local', weights=dinourl)
dinov3_vitl16.to(DEVICE)
dinov3_vitl16.eval()
...
def extract_features_batch(images: torch.Tensor) -> np.ndarray:
    ...
    with torch.no_grad():
        ...
        embeddings = dinov3_vitl16.forward_features(images.to(DEVICE))
...
def main():
    ...
    # ================================================================================
    # Brain Plane Classification Analysis (using raw DINOv3 features)
    # ================================================================================
    print("\n" + "="*60)
    print("BRAIN PLANE CLASSIFICATION ANALYSIS (RAW DINOv3 FEATURES)")
    print("="*60)
    
    # Collect raw DINOv3 features instead of PCA features
    print("\nCollecting raw DINOv3 features...")
    all_raw_features = []
    all_brain_data_raw = []
        ...
        for idx, row in tqdm(brain_df.iterrows(), total=len(brain_df), 
            ...
            if len(batch_images) == BATCH_SIZE or idx == brain_df.index[-1]:
                ...
                with torch.no_grad():
                    ...
                    embeddings = dinov3_vitl16.forward_features(batch_tensor.to(DEVICE))
    ...
    clf_name_raw = "Linear Discriminant Analysis (Raw DINOv3 Features)"
    ...
    with open(metrics_file_raw, 'w') as f:
        f.write("BRAIN PLANE CLASSIFICATION RESULTS (RAW DINOv3 FEATURES)\n")
        f.write("=" * 60 + "\n\n")
    ...
    print("\n" + "="*60)
    print("CONCLUSION: CAN DINOv3 DISTINGUISH BRAIN PLANES? (RAW FEATURES)")
    print("="*60)
