...
class FetalUS(ExtendedVisionDataset):
    ...
    class Split(Enum):
        ...
        def length(self) -> int:
            split_lengths = {
                ...
                FetalUS.Split.TEST: 22_172,     # Update with actual numbers (OLD -> 29_033)
            }
    ...
    #     """
    #     Override __getitem__ to properly handle DataAugmentationDINO transforms.
    #     DataAugmentationDINO expects a single image argument and returns a dict,
    #     not the standard (image, target) tuple that ExtendedVisionDataset expects.
    #     """
    #     try:
    #         image_path = self.get_image_path(index)
    #         image = Image.open(image_path).convert("RGB")
    ...
    #         if self.transforms is not None:
    #             # Apply DataAugmentationDINO transform which returns a dict
    ...
    #     except Exception as e:
    #         logger.warning(f"Error loading image at index {index}: {e}")
    #         # Return a minimal dict structure expected by collate function
    #         # This preserves the signature even for corrupted images
    #         dummy_image = Image.new('RGB', (224, 224), (0, 0, 0))
    #         if self.transforms is not None:
    #             return self.transforms(dummy_image)
    #         else:
    #             return dummy_image
